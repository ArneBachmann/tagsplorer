...
# TODO tokenize folder names and add each word!
  ...
  def stats(_):
    ...
    if idx.log >= 1:
      ...
      for n, t in sorted(byOccurrence.items()):  # TODO move above two lines into one dict comprehension
        info(" ".join(["  %d occurrences for %s " % (n, ", ".join(sorted(t, caseCompare)) if t[0] != '' else "/")]))
        if idx.log >= 2:  # TODO root element mapped to some paths, should not be! TODO all tags mapped to same folder
          for _t in sorted(t, caseCompare): ids = idx.tagdir2paths[idx.tagdirs.index(_t)]; debug("    %s: " % _t + ", ".join(["%s (%d)" % (idx.getPath(_i, _cache), _i) for _i in ids]))
