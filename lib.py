...
pathnorm = (lambda s: s.replace("\\", SLASH)) if sys.platform == 'win32' else ident  # as lambda to allow dynamic definition
...
class Config(object):
  ...
  def __init__(_):
    ...
    _.case_sensitive = sys.platform != 'win32'  # dynamic default unless specified differently in the config file
...
class Indexer(object):
  ...
  def _walk(_, aDir, parent, tags = []):
    ''' Recursive walk through folder tree.
        ...
        parent: the parent folder's index in the index (original case)
    ...
    if ignore:  # ignore this directory as tag, don't index contents
      ...
      if not _.cfg.reduce_case_storage and len(tags) >= 2 and _.tagdirs[tags[-1]] == caseNormalize(_.tagdirs[tags[-2]]): _.tagdir2paths[tags[-1]].pop()  # in case parent caller added true case and case-normalized version, remove both
      elif len(tags) > 0: _.tagdir2paths[tags[-1]].remove(parent)  # TODO recheck remove current folder tag from index (was introduced/appended by recursion parent/caller, unless we're in root). may leave empty list behind
    else:  # no ignore, only index extensions as additional tags
      for file in (ff for ff in files if os.extsep in ff[1:]):  # index only file extensions in this folder, without propagation to sub-folders
        ext = file[file.rindex(os.extsep):]
        ...
        if not _.cfg.reduce_case_storage and iext != ext:  # differs from normalized version: store normalized as well
          ...
          adds.append(i); _.tag2paths[i].append(parent)  # add current dir to index of that extension
    newtags = tags[1:-1] if ignore else [t for t in tags]  # if ignore: propagate all tags except current folder name to children, otherwise all (by shallow copy)
    ...
    for child in children:
      idxs = []  # first next index to use in recursion, no matter if true-case or case-normalized is used
      if not (sys.platform == 'win32' and _.cfg.reduce_case_storage):  # Windows: only store true-case if not reducing storage, Linux: always store
        if _.log >= 1: debug("Storing true-case folder name '%s'" % child)
        idxs.append(len(_.tagdirs))  # for this child folder, add one new element at next index (no matter if name already exists, because it always has a different parent)
      ...
      if sys.platform == 'win32' or (not _.cfg.reduce_case_storage and iname != child):  # Linux: only store case-normalized if not reducing storage, Windows: always store
      ...
      for idx in idxs: newtags.append(idx)  # temporary addition of first occurence of that tag string
      ...
      debug(_.tagdir2paths)
      for tag in frozenset(newtags + adds): _.tagdir2paths[tag].extend(idxs)  # store first occurence index only
      _._walk(aDir + SLASH + child, idxs[0], newtags)  # use original case index, not case-normalized one
      if not _.cfg.reduce_case_storage and len(newtags) >= 2 and _.tagdirs[newtags[-1]] == caseNormalize(_.tagdirs[newtags[-2]]): newtags.pop()  # remove case-normalized temporary folder tag addition
      newtags.pop()
